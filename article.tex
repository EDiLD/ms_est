...
We implemented the model using the gamlss packages \cite{}
...
Precipitation at the day before sampling increases $\mu$ and also $\pi$ for nearly all modeled compounds. 
The effects were less pronounced ($\pi$) or not directed ($\mu$) for precipitation at the day of sampling (Figure \ref{fig:fig5}, top).

Quarters 2-4 showed higher RQ than in Q1, except for the compounds  (Figure \ref{fig:fig5}, bottom)



\begin{figure}[ht]
  \includegraphics[width=0.95\textwidth]{figure5.pdf}
  \caption{Estimated coefficients and their 95\% CI for the model describe in equations \ref{eqn:eqn3} and \ref{eqn:eqn4} for each of the 24 analysed compounds (see Supplement Table . Left column: Effect on mean RQ; Right column: effect on the probability that RQ \textgreater 0.
  Coefficients where the CI encompasses zero are shown in grey.
  }
  \label{fig:fig5}
\end{figure}
