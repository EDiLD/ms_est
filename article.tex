...
We found substantial differences of the spectra of analysed compounds between federal states (Figure \ref{fig:figvar}).
...
Above this threshold the exceedances level off followed by a increase above 75\% (Figure \ref{fig:figrac}, left).

We could no detect any effect of catchment size on the number of RAC exceedances (Figure \ref{fig:figrac}, right) and no interaction between these two predictors.
...
\subsection{Pesticide pollution of small agricultural streams in germany}
...
Other compounds show exceedances only at less then 0.5\% of all samples (Figure \ref{fig:figpol}A).
% RAC
Insecticides showed most exceedances of RAC. 
For Thiacloprid, Imidacloprid and Chlorpyrifos the RAC was less than LOQ, therefore, all detections have a RQ \textgreater 1 (Figure \ref{fig:figpol}B). 
Highest RQ was observed for Chlorpyrifos (244), Dimoxystrobin(117) and Isoproturon (80). 
% TU
In 33\% of the samples no compounds no compounds were detected. 
The mean $log(TU_{max})$ for detects was -4.6.
2.6\% of the samples showed $log(TU_{max})$ values greater then -2 (Figure \ref{fig:figpol}C).
%Mixtures
In most samples (55.5\%) more then one compound was detected, with a maximum of 54 different compounds (Figure \ref{fig:figpol}D). 



\begin{figure}[h]
  \includegraphics[width=0.95\textwidth]{fig/pall.pdf}
  \caption{Overview on pesticide pollution of small agricultural streams. 
  A: 10 compounds with most frequent EQS exceedances. See B for color legend.
  B: 15 compounds with highest risk quotients.
  C: Distribution of $TU_{max}$ values. Values with $TU_{max} = 0$ are not displayed (3501 out of 10591 samples).
  D: Distribution of the number of detected compounds in a sample.
  }
  \label{fig:figpol}
\end{figure}
