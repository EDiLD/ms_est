...
Although there was high variability within the remaining cluster, this could not be further split (Figure~\ref{fig:fig2}, also Supplemental Figures~S3 and S4).
...
The probability to exceed LOQ increases 1.6-fold from 8.7\% to 13.5\% (Figure~\ref{fig:fig5}, top). % do_precip.R
...
There was a 2.5-folder higher probability of exceeding LOQ in $Q2$ (10.6\%) than in $Q1$ (4.6\%).
...
\begin{figure}[ht]
  ...
  \caption{Summarised coefficients (and their 95\% CI) for precipitation (top row) and quarter (bottom row) from a meta-analysis of the 24 modelled compounds. Left column: coefficients for mean RQ ($\mu$), right column: coefficients for probability to exceed LOQ ($\nu$). 
...
Moreover, all RACs were lower in our study (average = -0.71 $\mu g/L$, range = [-2.757; -0.005]). 
